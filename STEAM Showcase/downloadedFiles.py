...
mouseX, mouseY=(0, 0)
arduino = serial.Serial('COM5', 115200)#arduino mega communication
arduinoMega = serial.Serial('COM4',115200)
def write(mouseX,mouseY):
    resolution = (120,120)
    print(mouseX/resolution[0]*90)
    print(mouseY/resolution[1]*90)
    arduino.write(str(f"X{mouseX/resolution[1]*180}Y{mouseY/resolution[0]*180}").encode()) #set resolution
def reader():
    v = arduinoMega.readline()
    if v==1:
        return "ok"
